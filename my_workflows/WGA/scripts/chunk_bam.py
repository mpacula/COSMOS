...
class ChunkManager:
    ...
    def __init__(self,chunk_size,output_directory,header):
        ...
        self.chunks = [ Chunk(output_directory,header,1) ]
    ...
    def write(self,read):
        ...
        if chunk.count == self.chunk_size:
            chunk.close()
            self.chunks.append(Chunk(self.output_directory,self.header,self.current_chunk_num))
...
def splitBam(input_filename,output_directory,chunk_size):
    ...
    for rg in infile.header['RG']:
        ...
        new_header['RG'] = [rg]
        chunkManagers[rg['ID']] = ChunkManager(chunk_size,output_directory,new_header)
