...
class BWA_Align(step.Step):
    ...
    def none2many_cmd(self,data_dict):
        """
        :para: input_stage: a place holder and is ignored
        ...
        for f in data_dict:
            yield {
                    ...
                    'name' : step.dict2task_name({'sample':f['sample'],'lane':f['lane'],'fq_chunk':f['chunk'],'fq_pair':f['pair']})
                }
...
class BWA_Sampe(step.Step):
    ...
    def many2one_cmd(self,input_tasks,tags):
        task_r1 = input_tasks[0]
        task_r2 = input_tasks[1]
        return {
            'pcmd': r"""
                    {settings.bwa_path} sampe
                    -f  {{output_dir}}/{{outputs[sam]}}
                    -r "@RG\tID:{task_r1.tags[RG_ID]}\tLB:{task_r1.tags[RG_LIB]}\tSM:{task_r1.tags[sample]}\tPL:{task_r1.tags[RG_PLATFORM]}"
                    {settings.bwa_reference_fasta_path}
                    {task_r1.output_paths[sai]}
                    {task_r2.output_paths[sai]}
                    {task_r1.tags[fq_path]}
                    {task_r2.tags[fq_path]}
                """,
            'pcmd_dict':{'task_r1': task_r1,
                         'task_r2': task_r2 }
...
class CleanSam(step.Step):
    ...
    def one2one_cmd(self,input_task,input_type='bam'):
        return {
            'pcmd': r"""
                        {Picard_cmd}
                        I= {input}
            ...
            'pcmd_dict': {'Picard_cmd':get_Picard_cmd('CleanSam.jar',self.mem_req),
                          'input':input_task.output_paths[input_type]}
        }
...
class MergeSamFiles(step.Step):
    ...
    def many2one_cmd(self,input_tasks,tags,assume_sorted=True):
        INPUTs = "\n".join(["INPUT={0}".format(n.output_paths['bam']) for n in input_tasks])
        return {
            'pcmd': r"""
                        {Picard_cmd}
                        {INPUTs}
...
class MarkDuplicates(step.Step):
    ...
    def one2one_cmd(self,input_task,assume_sorted=True):
        return {
            'pcmd': r"""
                        {Picard_cmd}
                        I={input_task.output_paths[bam]}
                        O={{output_dir}}/{{outputs[bam]}}
...
class BuildBamIndex(step.Step):
    ...
    def one2one_cmd(self,input_task):
        return {
            'pcmd': r"""
                {Picard_cmd}
                INPUT={input_task.output_paths[bam]}
                OUTPUT={input_task.output_paths[bam]}.bai
            """,
...
class RealignerTargetCreator(step.Step):
    ...
    def multi_one2many_cmd(self,input_task_dict,intervals):
        for interval in intervals:
            yield {
                ...
                'pcmd_dict':{'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                             'input_bam': input_task_dict['MarkDuplicates'].output_paths['bam'],
                             'interval':interval},
...
class IndelRealigner(step.Step):
    ...
    def multi_one2one_cmd(self,input_task_dict,model='USE_READS'):
        
        """
        Expects rtc_stage to have been split by the same intervals.
        
        :param rtc_stage: the realigner target creator stage.  Its expected that it was parallelized by the same intervals as :param:`intervals`
        ...
        return {
            ...
            'pcmd_dict':{'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                         'model':model,
                         'input_bam': input_task_dict['MarkDuplicates'].output_paths['bam'],
                         'targetIntervals':input_task_dict['RealignerTargetCreator'].output_paths['targetIntervals'],
                         'interval':input_task_dict['RealignerTargetCreator'].tags['interval']}
        }
...
class BaseQualityScoreRecalibration(step.Step):
    ...
    def many2one_cmd(self,input_tasks,tags):
        INPUTs = ' '.join([ '-I {0}'.format(n.output_paths['bam']) for n in input_tasks ])
        return {
            'pcmd': self.pcmd,
            'pcmd_dict': {'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                          'INPUTs':INPUTs}
...
class PrintReads(step.Step):
    ...
    def multi_many2one_cmd(self,input_tasks_dict):
        INPUTs = ' '.join([ '-I {0}'.format(n.output_paths['bam']) for n in input_tasks_dict['IndelRealigner']])
        return {
            'pcmd': self.pcmd,
            'pcmd_dict': {'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                          'INPUTs':INPUTs,
                          'recal_file': input_tasks_dict['BaseQualityScoreRecalibration'][0].output_paths['recal']}
        }
...
class UnifiedGenotyper(step.Step):
    ...
    def many2many_cmd(self,input_tasks,tags,intervals):
        """
        UnifiedGenotyper takes as input all bams [sample1.bam,sample2.bam...sample3.bam]
        
        :param input_stage: sample level bams
        """
        input_bams = ' '.join([ '-I {0}'.format(n.output_paths['bam']) for n in input_tasks ])
...
class CombineVariants(step.Step):
    ...
    def many2one_cmd(self,input_tasks,tags,genotypeMergeOptions='UNSORTED'):
        """
        ...
        """
        INPUTs = "\n".join(["--variant {0}".format(n.output_paths['vcf']) for n in input_tasks])
        return {
                'pcmd': r"""
                        {GATK_cmd}
                        -T CombineVariants
...
class VariantQualityRecalibration(step.Step):
    ...
    def one2one_cmd(self,input_task,exome_or_wgs,haplotypeCaller_or_unifiedGenotyper='UnifiedGenotyper',inbreeding_coeff=True):
        """
        ...
        return {
            'pcmd' : cmd,
            'pcmd_dict': {'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                          'input_vcf':input_task.output_paths['vcf'],
...
class ApplyRecalibration(step.Step):
    ...
    def multi_one2one_cmd(self,input_task_dict,haplotypeCaller_or_unifiedGenotyper='UnifiedGenotyper'):
        """
        :param haplotypeCaller_or_unifiedGenotyper: choose from ('Haplotypecaller','UnifiedGenotyper')
        ...
        """
        #validation
        raw_variants_task = input_task_dict['CombineVariants']
        vqr_task = input_task_dict['VariantQualityRecalibration']
        glm = raw_variants_task.tags['glm']
        ...
        return {
            'pcmd': cmd,
            'pcmd_dict': {'GATK_cmd':get_GATK_cmd(mem_req=self.mem_req),
                          'input_vcf':raw_variants_task.output_paths['vcf'],
                         'input_tranches':vqr_task.output_paths['tranches'],
                         'input_recal':vqr_task.output_paths['recal'],}
        }
