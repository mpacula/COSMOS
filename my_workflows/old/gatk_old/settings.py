...
def get_Picard_cmd(jar,mem_req=2500):
    return 'java -Xmx{mem_req}m -Djava.io.tmpdir={tmp_dir} -jar {picard_jar}'.format(picard_jar = os.path.join(Picard_path,jar),mem_req=int(mem_req)*2,tmp_dir=tmp_dir)

def get_GATK_cmd(gatk_jar=os.path.join(GATK_path,'GenomeAnalysisTKLite.jar'),mem_req=2500,tmp_dir=tmp_dir):
    return 'java -Xmx{mem_req}m -Djava.io.tmpdir={tmp_dir} -jar {gatk_jar}'.format(gatk_jar = gatk_jar,mem_req=int(mem_req)*2,tmp_dir=tmp_dir)
